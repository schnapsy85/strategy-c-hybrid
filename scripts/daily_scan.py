...
def scheduled_time_guard(cfg: dict) -> bool:
    """Two UTC crons are used so Europe/Berlin DST is handled without hardcoding dates.

    The correct one lands in the 22:00 local hour. workflow_dispatch bypasses the guard.
    """
...
def main() -> None:
    ...
    # At 22:xx Europe/Berlin the U.S. regular session is already closed year-round.
    ...
    if not current_rows.empty:
        ...
        print(f"No grouped rows for {ny_today}; using latest cached completed trading day.")

    # Handles future constituent changes and any initially short ticker with 1 request/ticker.
    ...
    # Prefer the actual S&P 500 index (I:SPX) when the free Massive Indices Basic
    # entitlement is active. Fall back to SPY only if index history is unavailable.
    ...
    for ticker in eligible:
        ...
        if metrics.c1 or metrics.c2:
            m = meta.get(ticker, {})
            signals.append(build_signal(metrics, cfg, m.get("security", ticker), m.get("sector", "")))
    ...
    # C1 is preferred; then stronger relative position within the crossed band.
    signals.sort(key=lambda s: (0 if s["signal"] == "C1" else 1, s["ticker"]))
    watches = watches[:30]
    ...
    payload = {
        ...
        "status": "ok" if coverage >= 0.98 else "partial_data",
        "data_source": {
        ...
        "market_filter": {
            ...
            "close": proxy_close,
            "sma200": proxy_sma200,
            "pass": proxy_market_filter,
        },
        "signals": signals,
        "watch": watches,
        "metrics_by_ticker": all_metrics,
        "capital_reference_eur": float(cfg["capital_eur"]),
        "important": "Signals are research output, not orders. Recheck actual Strategy C capital, next-session gap filter and executable Scalable quote before any order preview."
    }
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Coverage: {len(eligible)}/{len(sp_tickers)} ({coverage:.1%})")
    print(f"Market filter ({market_ticker} > SMA200): {proxy_market_filter}")
